    ...
    public var data: UnsafeMutableBufferPointer<UnsafeMutablePointer<UInt8>?> {
        get {
            return withUnsafeMutableBytes(of: &native.pointee.data) { ptr in
                ptr.bindMemory(to: UnsafeMutablePointer<UInt8>?.self)
            }
        }
        set {
...
extension AVFrame.Flag: CustomStringConvertible, CustomDebugStringConvertible {
    ...
    private static let names = [
        ...
        .interlaced: ("interlaced", "AV_FRAME_FLAG_INTERLACED"),
        .key: ("key", "AV_FRAME_FLAG_KEY"),
        .interlaced: ("interlaced", "AV_FRAME_FLAG_INTERLACED"),
        .topFieldFirst: ("topFieldFirst", "AV_FRAME_FLAG_TOP_FIELD_FIRST"),
    ]
}
