...
public struct AVChapter {
    ...
    public init(id: Int, startSeconds: Double, endSeconds: Double, metadata: [String: String] = [:]) {
        self.init(
            id: id,
            timeBase: AVTimestamp.timebaseQ,
            start: Int64(startSeconds * Double(AVTimestamp.timebase)),
            end: Int64(endSeconds * Double(AVTimestamp.timebase)),
            metadata: metadata
        )
    }
}
